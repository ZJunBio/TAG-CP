...
def mol_feature(Mol):
    a_features = np.zeros((Mol.GetNumAtoms(), 39))
    b_features = np.zeros((Mol.GetNumBonds(), 10))
    edges = []
    for i, atom in enumerate(Mol.GetAtoms()):
        if atom.GetAtomicNum == 0:
            return None
        feature = atom_features(atom)
        a_features[i, :] = feature / sum(feature)
    for i, bond in enumerate(Mol.GetBonds()):
        bfeature = bond_features(bond)
        b_features[i, :] = bfeature / sum(bfeature)
        f = bond.GetBeginAtomIdx()
        b = bond.GetEndAtomIdx()
        edges.append([f, b])
    return a_features, b_features, edges 